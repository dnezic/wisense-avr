...
float compensate_humidity(float adc_h) {
	...
	var_h = (adc_h - (_calibration_h[3] * 64.0 + _calibration_h[4] / 16384.0 * var_h))
			* (_calibration_h[1] / 65536.0 * (1.0 + _calibration_h[5] / 67108864.0 * var_h * (1.0 + _calibration_h[2] / 67108864.0 * var_h)));
 ...
 */
BME280_ERROR_t read_all(BME280_DATA_t* data) {
	/*
	...
	data->temperature_integral = (uint8_t) temperature;
	data->temperature_decimal = (uint8_t) ((temperature - (uint8_t) temperature) * 100);
	float pressure = compensate_pressure(raw_press_f);

	data->pressure_1 = ((uint16_t) temperature) & 0xFF;
	data->pressure_2 = (((uint16_t) temperature) >> 8) & 0xFF;
	data->pressure_3 = (uint8_t) ((temperature - (uint16_t) temperature) * 100);
